...
module.exports = {
  ...
  create(context) {
    ...
    let sourceCodeText = context.getSourceCode().text;
    ...
    return {
      AssignmentExpression(node) {
        ...
        let packageJsonText = sourceCodeText.substring(...packageJsonNode.range);
        ...
        if (packageJsonText !== sortedText) {
          context.report({
            ...
            fix(fixer) {
              if (!matches) {
                matches = sourceCodeText.match(/(\r?\n)$/m) || [];
              }
              let suffix = matches[1] || '';
              return fixer.replaceText(node, sortedText + suffix);
            }
          });
        }
      }
